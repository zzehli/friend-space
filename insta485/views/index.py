...
def query_home_post(user):
    """
    With a given user, query all the tables for relevant post info of the user 
    and ppl they are following
    """
    # make sure to check session first before call this func
    # query  posts
    ...
    following_list = [x['username2'] for x in following]
    # add the user to the list
    following_list.append(user)
    # format list variable in sqlite3 module
    # https://stackoverflow.com/questions/64195350/searching-in-list-in-python-sqlite3
    cur = insta485.model.get_db().execute(
        'SELECT * \
        FROM posts \
        WHERE owner IN ({0}) \
        ORDER BY postid DESC'
        .format(', '.join('?' for _ in following_list)), following_list
    )
    post_list = cur.fetchall()
    posts = {x['postid']:x for x in post_list}
    cur = insta485.model.get_db().execute(
        "SELECT * FROM likes;"
    )
    likes = cur.fetchall()
    ...
    # post_list = [x['postid'] for x in posts \
    #             if x['owner'] in following_list or x['owner'] == user]
    # own_post = [x['postid'] for x in posts if x['owner'] == user]
    following_list = [x['username2'] for x in following]
    likes_count = {}
    for i in likes:
        # whether the user liked the post
        if i['owner'] == user and i['postid'] in posts.keys():
            posts[i['postid']]['liked'] = True

        if i['postid'] in likes_count:
            likes_count[i['postid']] += 1
        else:
            likes_count[i['postid']] = 1
    post_info = {}
    # add # of likes, commments from other tables
    for i in posts.values():
        if 'liked' not in i.keys():
            i['liked'] = False
        if i['owner'] in following_list or i['owner'] == user:
            i['likes'] = likes_count[i['postid']] if i['postid'] in likes_count.keys() else 0
            i['comments'] = [x for x in comments if x['postid'] == i['postid']]
            formated_time = arrow.get(i['created'], 'YYYY-MM-DD HH:mm:ss')
            i['timestamp'] = formated_time.humanize()
            post_info[i['postid']] = i
    return post_info
...
def index():
    """Display / route."""
    # check session, if logged in directs to the index page
    if 'user' in flask.session:
        post_info = query_home_post(flask.session['user'])
        user_info = query_user_following(flask.session['user'])
        cur = insta485.model.get_db().execute(
                "SELECT username, filename \
                FROM users \
                WHERE username = (?)", (flask.session['user'],)            
        )
        self_info = cur.fetchall()
        user_info.append(self_info[0])
        for post in post_info.values():
            for user in user_info:
                if user['username'] == post['owner']:
                    post['owner_img_url'] = user['filename']
        context = {"posts": post_info}
